...
struct __attribute__((packed)) tss_entry_struct {
    ...
    uint32_t es;       uint32_t cs;   uint32_t ss;   uint32_t ds;
    uint32_t fs;       uint32_t gs;   uint32_t ldt;
    uint16_t trap;     uint16_t iomap_base;
};
typedef struct tss_entry_struct tss_entry_t;

// Share the global variable and functions with other source files
