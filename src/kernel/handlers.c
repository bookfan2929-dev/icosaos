...
void keyboard_handler(void) {
    ...
    if (!(scancode & 0x80)) {
        if (scancode < sizeof(scancode_to_ascii)) {
            ...
            if (ascii != 0) {
                // Instantly echo characters to your working VGA display driver
                // (White text foreground, Dark Blue background matching your clear state)
                vga_put_char(ascii, 0x00FFFFFF, 0x000000FF);
            }
        }
    }
